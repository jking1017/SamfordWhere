...
class ViewController: UIViewController {
  ...
  typealias classLabel = (String, Double)
  ...
  func predict(pixelBuffer: CVPixelBuffer) {
    ...
    if let resizedPixelBuffer = resizePixelBuffer(pixelBuffer,
      ...
      // Give the resized input to our model.
        if let classlabel = try? model.prediction(data: resizedPixelBuffer) {
        let top5 = top(5, classlabel.loss)
  ...
  func show(results: [classLabel], elapsed: CFTimeInterval) {
    var s: [String] = []
